...
const { getArticle, getArticleCategories, createArticle, updateArticle, createArticleCategory, uploadImage, getTags } = useApi()
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        let result
        if (isEdit.value) {
          result = await updateArticle(route.params.id, form)
