...
const mapStrapiArticle = (item) => {
  ...
  const galleryImages = attrs.blocks
    ...
    .map(b => {
      const url = b.file?.data?.attributes?.url
  ...
  return {
    ...
    tags: [], // Schema has no tags
    images: galleryImages,
    // Keep raw for updates if needed, but ideally we manage IDs separately
    rawCategory: attrs.category?.data
  }
}
...
export const useApi = () => {
  ...
  const getArticleCategories = async () => {
    try {
      ...
      return response.data.data.map(mapStrapiCategory)
    } catch (error) {
  ...
  const createArticleCategory = async (data) => {
    try {
      // payload: { data: { name, slug, description } }
      const payload = {
        name: data.name,
        slug: data.slug,
        description: `Order: ${data.order}, Active: ${data.isActive}` // Store extras in description? Or just ignore
      }
  ...
  const updateArticleCategory = async (id, data) => {
    try {
      const payload = {
        name: data.name,
        slug: data.slug
        // Ignoring order/isActive for now as schema doesn't support them explicitly
      }
  ...
  const reorderArticleCategories = async (newOrder) => {
    try {
      // Strapi bulk update usually requires a custom controller or individual updates
      // Here we simulate it or assume a custom endpoint if needed. 
      // For now, let's update individually for a simple implementation
      await Promise.all(newOrder.map((id, index) =>
        api.put(`${STRAPI_URL}/api/article-categories/${id}`, { data: { order: index + 1 } })
      ))
      return true
    } catch (error) {
      newOrder.forEach((id, index) => {
